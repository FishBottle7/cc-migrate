 ...
 *      6. buildMigrateApi 产出恰好 3 个 method（list-sources/preview/import）。
 *      7. registerMigrateRoutes：mock webServer（register 返回 disposer——
 *         dsh-host-webserver 契约）→ 1 条 prefix 路由 @ /cc-migrate/api。
 *      8. 路由 handler 全链：mock req/res（结构同形——asyncIterator 请求
 *         体、writeHead/end 响应），临时 claude 库（core 写入管线造）的
 *         list 返回 JSON {ok:true,value}。
 *      9. fence 违例：跨站 Host/Origin 组合 403；GET 405；未知 method 404；
 *         坏 JSON 400（信封字段齐全——GUI 渲染层吃这些结构）。
 *     10. import method 写临时 DSH root（read-old-write-new：两个全新 id）。
 *
...
console.log(`[A3b] factory returns { apply, inject: ${JSON.stringify(modExports.inject)} }`);
...
assert.ok(html.includes('Claude Code'), 'tool cards rendered');
...
console.log(`[A5] tab.component renders headless via react-dom/server (${html.length} chars, header/tool cards/input row/empty state present)`);
...
for (const method of ['list-sources', 'preview', 'import']) {
  assert.equal(typeof api[method], 'function', `api method "${method}" exists`);
}
assert.equal(Object.keys(api).length, 3, 'exactly 3 api methods');
console.log('[B6] buildMigrateApi -> 3 methods: list-sources, preview, import');
...
console.log('\nCLIENT SMOKE OK — bundle shape + factory execution + tab registration + headless render; fenced host routes verified in temp roots.');
