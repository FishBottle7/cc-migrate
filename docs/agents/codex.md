...
- **分类通道一致性修订（2026-09-06）**：legacy 文本表与官方 kinds 通道对同一内容定性对齐——`<subagent_notification>`/`<recommended_plugins>`/`<skill>` 在 legacy 表改标 `synthetic:true`（官方通道三者本就 synthetic：前两个在 SYNTHETIC_KINDS、`skills.instructions` 命中 `.instructions` 前缀；无 kinds 的旧行此前按用户消息处理属偏差，真机 2026-01/02 实样确认存在）。同时官方通道给 `agents_md.instructions` 加 `.instructions` 前缀豁免——§8「非 synthetic」决策对带 kinds 的 AGENTS.md 行同样生效，两通道在此完全一致（真机 2026-09 顶层 AGENTS.md 行带 passthrough 但无 `content_item_kinds`，本就走 legacy 表）。
